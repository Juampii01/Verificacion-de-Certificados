...
const LAYOUT = {
  ...
  // Centrado en el espacio libre real entre el QR (borde derecho ~x=220) y el panel de datos (borde izquierdo x=674.6),
  // no en el centro de la página entera.
  name: { centerX: 447, y: PAGE_H - 314, size: 26 },
...
export async function certificatePdf(cert) {
  ...
  if (hasBg) {
    ...
    // El hueco del nombre entre "reconoce que" y "ha completado" tiene ~430pt de ancho libre
    // (entre el QR a la izquierda y la columna de datos a la derecha); si el nombre no entra
    // al tamaño base, se achica hasta que quepa.
    ...
    page.drawText(fmtDate(cert.issue_date), {
      x: LAYOUT.issueDate.x, y: LAYOUT.issueDate.y, size: LAYOUT.issueDate.size, font, color: NAVY,
    });
  } else {
