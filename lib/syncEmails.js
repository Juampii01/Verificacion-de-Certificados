...
export async function applyEmailMatches(pairs) {
  ...
  for (const { certificate_number, email } of pairs) {
    // El count de { count: "exact" } en .select() encadenado a un update()
    // no vino confiable (devolvía 0 con la fila igual actualizada) — se usa
    // el length de las filas devueltas por .select(), que sí refleja cuántas
    // filas matcheó y actualizó el update.
