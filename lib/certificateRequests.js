...
import { certificatePdf, certificateFilename } from "./pdf.js";
import { sendCertificateReadyEmail, sendCertificateRejectedEmail } from "./email.js";

export const MAX_NAME_LENGTH = 45;

// Fecha en la que se completó el día 4 del challenge — es la fecha de
// finalización real de todos los certificados generados por este flujo,
// no la fecha en la que se pide/aprueba el certificado.
const DAY4_COMPLETION_DATE = "2026-07-03";
...
export async function submitCertificateRequest({ first_name, last_name, full_name, email }) {
  ...
  if (match) {
    const result = await createCertificates([{
      ...
      issue_date: DAY4_COMPLETION_DATE,
      hours: 16, signed_by: "Santo González",
    }]);
