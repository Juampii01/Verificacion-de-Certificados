...
export async function sendCertificateReadyEmail(to, { full_name, certificate_number, pdfBytes, pdfFilename }) {
  const url = verifyUrl(certificate_number);
  const attachedNote = pdfBytes
    ? "Te lo adjuntamos en PDF en este mismo correo. También podés verificarlo"
    : "Podés verificarlo";
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto">
      <h2 style="color:#152978">Tu certificado GovBidder está listo</h2>
      <p>Hola ${full_name},</p>
      <p>Tu certificado <b>${certificate_number}</b> ya está activo. ${attachedNote} en el
  ...
  return client().emails.send({
    ...
    html,
    ...(pdfBytes ? { attachments: [{ filename: pdfFilename || `${certificate_number}.pdf`, content: Buffer.from(pdfBytes) }] } : {}),
  });
}
